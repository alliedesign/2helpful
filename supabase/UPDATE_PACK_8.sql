-- ============================================================
--  UPDATE PACK 8 — Real reviews only (no generic/seed ratings)
--  Ratings and counts now come ONLY from real reviews. Listings
--  with no reviews return rating = null and review_count = 0,
--  so the site can show "No reviews yet" instead of a fake number.
...
returns table (
  ...
  with scored as (
    select
      ...
      s.avg_rating as rating,                         -- real average, or null
      coalesce(s.review_total, 0) as review_count,    -- real count, or 0
